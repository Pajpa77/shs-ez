...
export const TacticalWeatherOverlay: React.FC<TacticalWeatherOverlayProps> = ({
  ...
  if (!weatherReport && loading) {
    return (
      <div className="bg-[#1E293B]/95 backdrop-blur-md border border-slate-300 dark:border-slate-700 rounded-2xl p-3 text-slate-900 dark:text-slate-200 shadow-xl flex items-center gap-2 text-xs font-mono">
  ...
  const scentBadgeColor = {
    ...
    moderate: 'bg-slate-50 dark:bg-[#0F172A]mber-500/20 text-amber-300 border-amber-500/40',
    difficult: 'bg-orange-500/20 text-orange-300 border-orange-500/40',
    critical: 'bg-red-500/20 text-red-300 border-red-500/40',
  }[k9SearchImpact.scentConditions];

  return (
    <div className="bg-[#1E293B]/95 backdrop-blur-md border border-slate-300 dark:border-slate-700 rounded-2xl shadow-2xl text-black dark:text-slate-100 flex flex-col overflow-hidden text-xs transition-all select-none">
      {/* Header Bar */}
      <div className="flex items-center justify-between gap-2 p-2.5 sm:p-3 bg-white dark:bg-slate-900/80 border-b border-slate-300 dark:border-slate-700/80">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">({timestamp})</span>
            </div>
            <div className="flex items-center gap-2 text-[11px] text-slate-700 dark:text-slate-300 font-mono">
              ...
              <span className="text-slate-500 dark:text-slate-400">Gefühlt: {current.apparentTemperature}°C</span>
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button
            ...
            onClick={loadWeather}
            disabled={loading}
            className="p-1.5 rounded-lg bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 hover:text-white transition cursor-pointer border border-slate-300 dark:border-slate-700"
          ...
          {onClose && (
            <button
              type="button"
              onClick={onClose}
              className="p-1.5 rounded-lg bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 hover:text-white transition cursor-pointer border border-slate-300 dark:border-slate-700"
              title="Schließen"
            >
      ...
      {expanded && (
        <div className="p-3 space-y-3 animate-in fade-in duration-150">
          {/* Tabs */}
          <div className="flex rounded-xl bg-white dark:bg-slate-900/90 p-1 border border-slate-200 dark:border-slate-800 text-[11px] font-mono">
            <button
              type="button"
          ...
          {activeTab === 'current' && (
            <div className="space-y-2">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 text-[11px] font-mono">
                <div className="bg-white dark:bg-slate-900/60 p-2 rounded-xl border border-slate-200 dark:border-slate-800 flex items-center gap-2">
                  <Wind className="w-4 h-4 text-cyan-400 shrink-0" />
                  <div>
                    <div className="text-slate-500 dark:text-slate-400 text-[9px] uppercase">Wind / Böen</div>
                    <div className="font-bold text-white">
                      {current.windSpeed} <span className="text-[9px] font-normal text-slate-500 dark:text-slate-400">km/h</span>
                ...
                <div className="bg-white dark:bg-slate-900/60 p-2 rounded-xl border border-slate-200 dark:border-slate-800 flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-slate-500 dark:text-slate-400 text-[9px] uppercase">Luftfeuchte</div>
                ...
                <div className="bg-white dark:bg-slate-900/60 p-2 rounded-xl border border-slate-200 dark:border-slate-800 flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-slate-500 dark:text-slate-400 text-[9px] uppercase">Niederschlag</div>
                ...
                <div className="bg-white dark:bg-slate-900/60 p-2 rounded-xl border border-slate-200 dark:border-slate-800 flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-slate-500 dark:text-slate-400 text-[9px] uppercase">Bewölkung</div>
          ...
          {activeTab === 'k9' && (
            <div className="space-y-2.5">
              ...
              <div className="bg-white dark:bg-slate-900/70 p-2.5 rounded-xl border border-slate-200 dark:border-slate-800 space-y-1.5">
                ...
                {k9SearchImpact.dogHeatRisk !== 'none' && (
                  <div className="flex items-center gap-1.5 text-[11px] text-amber-300 bg-slate-50 dark:bg-[#0F172A]mber-950/40 p-1.5 rounded-lg border border-amber-800/60 font-mono">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded-full border text-[10px] font-mono ${
                      k9SearchImpact.droneFlightCondition === 'safe'
                        ...
                        ? 'bg-slate-50 dark:bg-[#0F172A]mber-500/20 text-amber-300 border-amber-500/40'
            ...
            <div className="space-y-1.5">
              <div className="grid grid-cols-6 gap-1 text-center font-mono">
                {hourly.map((h, idx) => (
                  <div
                    key={idx}
                    className="bg-white dark:bg-slate-900/80 p-1.5 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col items-center gap-1"
                  >
              ...
              <p className="text-[10px] text-slate-500 dark:text-slate-400 text-center font-mono pt-1">
