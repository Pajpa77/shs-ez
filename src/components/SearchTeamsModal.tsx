...
export const SearchTeamsModal: React.FC<SearchTeamsModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[5000] flex items-center justify-center p-3 sm:p-6 bg-slate-100 dark:bg-slate-950/85 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="bg-[#1E293B] border border-slate-300 dark:border-slate-700 rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl text-black dark:text-slate-100 overflow-hidden">
        {/* Header */}
        <div className="px-5 py-4 bg-white dark:bg-slate-900 border-b border-slate-300 dark:border-slate-700 flex items-center justify-between shrink-0">
          ...
          <button
            onClick={onClose}
            className="p-2 text-slate-500 dark:text-slate-400 hover:text-white hover:bg-slate-50 dark:bg-slate-800 rounded-xl transition cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
                ...
                {teams.length > 0 ? (
                  teams.map((team) => {
                    ...
                    return (
                      <div
                        key={team.id}
                        className="bg-white dark:bg-slate-900/90 border border-slate-300 dark:border-slate-700/80 rounded-2xl p-4 space-y-3 shadow-lg flex flex-col justify-between"
                      >
                        <div className="space-y-2">
                          ...
                          {/* Leader */}
                          <div className="bg-slate-50 dark:bg-slate-800/80 p-2.5 rounded-xl border border-slate-300 dark:border-slate-700/60 text-xs">
                            ...
                            <div className="font-bold text-white flex items-center justify-between">
                              ...
                              <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">{leader?.organization || 'Rettungsdienst'}</span>
                            </div>
                          </div>

                          {/* Members & External Volunteers */}
                          <div className="grid grid-cols-2 gap-2 text-xs">
                            <div className="bg-slate-100 dark:bg-slate-950/60 p-2 rounded-xl border border-slate-200 dark:border-slate-800">
                              <span className="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase font-mono block mb-1">
                                Sucher ({members.length}):
                              </span>
                              <div className="flex flex-wrap gap-1">
                                {members.length > 0 ? (
                                  members.map((m) => (
                                    <span key={m.id} className="px-2 py-0.5 rounded bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-200 text-[10px] font-mono">
                                      {m.callSign}
                                    </span>
                                  ))
                                ) : (
                          ...
                          {team.notes && (
                            <p className="text-[11px] text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-950/50 p-2 rounded-lg border border-slate-200 dark:border-slate-800 italic">
                              "{team.notes}"
                            </p>
                          )}
                        </div>
                        ...
                        {currentUser?.role === 'admin' && (
                          <div className="mt-4 pt-3 border-t border-slate-200 dark:border-slate-800 flex items-center justify-end gap-2">
                            <button
                              ...
                              className="px-3 py-1.5 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:bg-slate-700 text-slate-900 dark:text-slate-200 rounded-xl text-xs font-bold transition cursor-pointer border border-slate-300 dark:border-slate-700 flex items-center gap-1"
                            >
                  ...
                  <div className="col-span-full py-12 text-center text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800">
                    <Users className="w-12 h-12 text-slate-600 mx-auto mb-2" />
                    <p className="font-bold text-slate-700 dark:text-slate-300">Noch keine Suchtrupps gebildet</p>
            ...
            <form onSubmit={handleSaveTeam} className="space-y-4">
              <div className="flex items-center justify-between border-b border-slate-300 dark:border-slate-700 pb-3">
                ...
                <button
                  ...
                  className="text-xs text-slate-500 dark:text-slate-400 hover:text-white cursor-pointer font-mono"
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1 font-mono uppercase tracking-wider text-[11px]">
                    Trupp-Name:
                  </label>
                  <input
                    type="text"
                      ...
                      <button
                        ...
                        className={`p-2 rounded-xl border text-left transition cursor-pointer flex items-center justify-between text-xs font-mono ${
                          isSelected
                            ...
                            : 'bg-slate-100 dark:bg-slate-950/60 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:bg-slate-800'
                        }`}
                      >
                        <div>
                          <div className="font-bold truncate">{u.name}</div>
                          <div className="text-[10px] text-slate-500 dark:text-slate-400">{u.callSign}</div>
                        </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full sm:w-48 px-3 py-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-black dark:text-slate-100 text-xs focus:outline-none focus:border-blue-500 font-mono"
                />
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-300 dark:border-slate-700">
                <button
                  type="button"
                  onClick={() => setIsEditing(false)}
                  className="px-4 py-2 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl font-bold text-xs cursor-pointer border border-slate-300 dark:border-slate-700"
                >
                  Abbrechen
                </button>
                <button
        ...
        {/* Footer */}
        <div className="px-5 py-3 bg-white dark:bg-slate-900 border-t border-slate-300 dark:border-slate-700 flex items-center justify-between shrink-0">
          <div className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">
            Suchtrupp-Verwaltung • {teams.length} Trupps aktiv
          </div>
          <button
            onClick={onClose}
            className="px-4 py-2 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:bg-slate-700 text-slate-900 dark:text-slate-200 rounded-xl font-bold text-xs cursor-pointer border border-slate-300 dark:border-slate-700"
          >
            Schließen
          </button>
        </div>
