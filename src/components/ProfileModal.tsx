...
export const ProfileModal: React.FC<ProfileModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[5000] flex items-center justify-center p-3 sm:p-4 bg-slate-100 dark:bg-slate-950/80 backdrop-blur-md overflow-y-auto font-sans">
      <div className="bg-[#1E293B] border border-slate-300 dark:border-slate-700 rounded-2xl shadow-2xl w-full max-w-xl text-black dark:text-slate-100 overflow-hidden my-auto animate-in fade-in zoom-in-95 duration-200">
        {/* Header */}
        ...
        <form onSubmit={handleSave} className="p-5 space-y-4 max-h-[80vh] overflow-y-auto text-xs">
              ...
              <div className="flex-1 space-y-2 w-full">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold transition cursor-pointer flex items-center gap-1.5 font-mono text-[11px] border border-slate-300 dark:border-slate-700"
                  >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1 font-mono uppercase tracking-wider">Mein Name *:</label>
              <input
                type="text"
                required
                value={name}
            ...
            <div>
              <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1 font-mono uppercase tracking-wider">Funkrufname (Callsign):</label>
              <input
                type="text"
                value={callSign}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                {currentUser?.role === 'admin' && !memberId.trim() && (
                  <button
                    type="button"
              ...
              <input
                ...
                onChange={(e) => currentUser?.role === 'admin' && setMemberId(e.target.value)}
                placeholder="Ausweisnummer (z.B. SHS-482910)..."
                className={`w-full px-3 py-2 rounded-xl border text-xs font-mono ${
                  currentUser?.role === 'admin'
                    ? 'bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 text-black dark:text-slate-100 focus:outline-none focus:border-blue-500'
                    : 'bg-slate-100 dark:bg-slate-950/80 border-slate-300 dark:border-slate-800 text-slate-400 cursor-not-allowed'
                }`}
                title={currentUser?.role === 'admin' ? 'Ausweisnummer bearbeiten' : 'Ausweisnummer kann nur von Administratoren in der Accountverwaltung geändert werden'}
              />
            </div>

            <div>
              <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1 font-mono uppercase tracking-wider">Mobiltelefon:</label>
              <input
                type="text"
                value={phone}
          ...
          <div className="bg-white dark:bg-slate-900/80 p-3.5 rounded-xl border border-slate-300 dark:border-slate-700 space-y-2 font-mono">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
            ...
            {hasHardwareBattery ? (
              ...
              <div className="space-y-1.5 pt-1">
                <div className="flex items-center justify-between text-[10px] text-slate-500 dark:text-slate-400">
                  ...
                  <span className="font-bold text-slate-900 dark:text-slate-200">{batteryLevel}%</span>
                </div>
                <input
                  type="range"
                  min="5"
                  ...
                  className="w-full h-1.5 bg-slate-50 dark:bg-slate-800 rounded-lg appearance-none cursor-pointer accent-emerald-500"
                />
              ...
              <div className="pt-2 border-t border-slate-200 dark:border-slate-800 space-y-2.5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  <div className="space-y-1">
                    <label className="block text-[10px] text-slate-700 dark:text-slate-300 uppercase tracking-wider font-bold">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full px-3 py-1.5 pr-8 rounded-xl bg-slate-100 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-black dark:text-slate-100 text-xs focus:outline-none focus:border-amber-500 font-mono"
                      />
                      <button
                        type="button"
                        onClick={() => setShowNewPassword(!showNewPassword)}
                        className="absolute right-2 top-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-slate-200"
                      >
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-black dark:text-slate-100 text-xs focus:outline-none focus:border-amber-500 font-mono"
                    />
                  </div>
                </div>
          ...
          {/* Footer Actions */}
          <div className="pt-3 border-t border-slate-300 dark:border-slate-700 flex items-center justify-end gap-2">
            <button
              type="button"
              onClick={onClose}
